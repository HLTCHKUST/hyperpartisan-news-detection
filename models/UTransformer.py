...
from utils.feature_utils import gen_embeddings
...
class UTransformer(nn.Module):
    ...
    def __init__(self, vocab, embedding_size, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
                 filter_size, max_length=500, input_dropout=0.0, layer_dropout=0.0, 
                 attention_dropout=0.0, relu_dropout=0.0, use_mask=False, act=False ):
        super(UTransformer, self).__init__()
        self.embedding_dim = embedding_size
        self.vocab = vocab
        self.emb = share_embedding(self.vocab, constant.pretrain_emb, embedding_size)
    ...
    def forward(self, X, X_lengths=None):
        ...
        return torch.sum(logit,dim=1)/logit.size(1)

def share_embedding(vocab, pretrain, embedding_size):
    embedding = nn.Embedding(vocab.n_words, constant.emb_dim, padding_idx=constant.PAD_idx)
    if(pretrain):
        if embedding_size == 100:
            pre_embedding = gen_embeddings(vocab, embedding_size, "../glove/glove.6B.100d.txt")
        else:
            pre_embedding = gen_embeddings(vocab, embedding_size, "../glove/glove.42B.300d.txt")
