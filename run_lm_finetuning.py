...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, tokenizer, seq_len, encoding="utf-8", corpus_lines=None, on_memory=True):
        ...
        if on_memory:
            ...
            with open(corpus_path, "r", encoding=encoding) as f:
                for line in tqdm(f, desc="Loading Dataset On Memory", total=corpus_lines):
                    line = line.strip()
                    if line == "":
                        # document is empty then continue
    ...
    def get_random_line(self):
        ...
        for _ in range(10):
            if self.on_memory:
                # make sure len(rand_doc) > 0
                while True:
                    rand_doc_idx = random.randint(0, len(self.all_docs)-1)
                    rand_doc = self.all_docs[rand_doc_idx]
                    if len(rand_doc) > 0: break
    ...
    if args.do_train:
        ...
        for e in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in pbar:
                ...
                nb_tr_steps += 1
                pbar.set_description("(Epoch {}) TRAIN LOSS:{:.4f}".format((e+1), tr_loss/float(nb_tr_steps)))
