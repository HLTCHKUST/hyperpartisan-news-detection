...
def eval_bert(article_model, title_model, LR, loader, tokenizer, use_add_feature_flag=False, writer=None, global_steps=None, isTest=False):
    pred = []
    gold = []
    id=[]
    if use_add_feature_flag:
...
def eval_bert_with_chunked_data(article_model, title_model, LR, loader, tokenizer, ids_dict, writer=None, global_steps=None, isTest=False):
...
def predict(article_model, title_model, lstm_article, lstm_title, LR, loader, k=None, name="", print_pred=False, output_path="test/inputRun"):
    ...
    with open('{}/{}'.format(output_path, f_name), 'w') as result_file:
        if not constant.use_bert:
            ...
            tokenizer = torch.load("/home/yeon-zi/fakenews_submission/bert_model/bert_tokenizer.model")
            for X, x_len, tit, tit_len, _, id_ in tqdm(loader):
                X = [tokenizer.convert_tokens_to_ids(tokenizer.tokenize(item)) for item in X]
                tit = [tokenizer.convert_tokens_to_ids(tokenizer.tokenize(item)) for item in tit]
                # padding
